...
document.addEventListener('DOMContentLoaded', () => {
  if (isLocalMode) {
    showToast('Running in standalone mode (saved in browser local storage)', 'info');
  }
  if (token) {
    checkAuth();
  } else {
...
async function handleLocalRoute(url, options) {
  ...
  // Initialise database in local storage if not exists
  let db = JSON.parse(localStorage.getItem('kinship_db'));
  
  // Migration check: Reset database if it contains the old unsalted hashes to avoid broken credentials
  const hasOldHash = db && db.users && db.users[0] && db.users[0].passwordHash === 'c775e7b757ede630cd0aa1113bd102661ab38829ca52a6422ab782862f268646';
  
  if (!db || hasOldHash) {
    db = getSeedDatabase();
    localStorage.setItem('kinship_db', JSON.stringify(db));
    // Clear token to force fresh login if database was reset
    localStorage.removeItem('kinship_token');
    token = null;
  }
  
  // Helper to save database updates
  const saveDb = () => localStorage.setItem('kinship_db', JSON.stringify(db));
